...
import "./appkit";
import { ARC, NFT_ABI, NFT_CONTRACT, OPENSEA_URL, X_URL } from "./config";
import "./styles.css";
...
const rarityCards = [
  ["/assets/rarity-normal-01.svg", "NORMAL", "4,980 SUPPLY"],
  ["/assets/rarity-normal-02.svg", "NORMAL", "TRAIT VARIANT"],
  ["/assets/rarity-normal-03.svg", "NORMAL", "TRAIT VARIANT"],
  ["/assets/rarity-zombie.svg", "ZOMBIE", "10 SUPPLY"],
  ["/assets/rarity-alien.svg", "ALIEN", "10 SUPPLY"]
];

function RarityGallery() {
  return <section id="rarities" className="section rarities"><div className="rarityhead"><div><div className="eyebrow">Rarity gallery</div><h2>THE UNCOMMON<br/>ONES.</h2></div><p className="copy">Every Raccoon is an on-chain SVG. The 20 specials are fixed: ten Zombies and ten Aliens across the full 5,000 supply.</p></div><div className="raritygrid">{rarityCards.map(([src,type,label])=><article className={`raritycard ${type.toLowerCase()}`} key={src}><img src={src} alt={`${type} StonkRaccoon`}/><div><b>{type}</b><span>{label}</span></div></article>)}</div></section>;
}

function LegacyHome({ go }) {
...
function Home({ go }) {
  return <>
    <Header go={go}/>
    <main>
      <section className="hero"><div><div className="eyebrow">Fully on-chain / Arc Mainnet</div><h1>STREET SMART.<br/>ON-CHAIN.</h1><p>5,000 pixel raccoons built natively for Arc. Every trait, every special, every PFP lives on-chain.</p><div className="actions"><button className="mint" onClick={() => go("/mint/")}>MINT A RACCOON</button><button className="secondary" onClick={() => go("/docs/")}>READ DOCS ↗</button></div></div><div className="art"><img src="/assets/raccoon-headset.svg" alt="Pixel StonkRaccoons character"/></div></section>
      <section className="metricbar"><div className="metric"><b>5,000</b><span>On-chain raccoons</span></div><div className="metric"><b>20</b><span>Zombie + Alien specials</span></div><div className="metric"><b>5,000 $RACC</b><span>Per minted NFT</span></div></section>
      <section id="collection" className="section split"><div><div className="eyebrow">The collection</div><h2>TRAITS WITH A LITTLE ATTITUDE.</h2></div><div><p className="copy">4,980 normal StonkRaccoons plus exactly 10 Zombies and 10 Aliens. Six fur types, six glasses, twelve fits, ten headwear pieces and a stack of headphones, chains and earrings.</p><div className="gallery"><img src="/assets/raccoon-01.svg" alt="StonkRaccoon"/><img src="/assets/raccoon-02.svg" alt="StonkRaccoon"/><img src="/assets/raccoon-base.svg" alt="StonkRaccoon"/></div><div className="specials"><span><b>10</b> ZOMBIES</span><span><b>10</b> ALIENS</span><span><b>100%</b> ON-CHAIN SVG</span></div></div></section>
      <RarityGallery/>
      <section id="epochs" className="section split"><div><div className="eyebrow">Mint curve</div><h2>10 EPOCHS.<br/>500 EACH.</h2></div><div><p className="copy">Mint price moves up by 0.25 USDC after each 500-raccoon epoch. Wallet mint capacity refreshes every epoch.</p><div className="curve"><svg viewBox="0 0 900 220" preserveAspectRatio="none"><polyline points="0,210 100,190 200,168 300,145 400,122 500,100 600,77 700,55 800,32 900,10"/><circle cx="0" cy="210" r="7"/><circle cx="900" cy="10" r="7"/></svg><small>EPOCH 1 — FREE → EPOCH 10 — 2.25 USDC</small></div><div className="epochs">{["FREE · 1",".25 · 20",".50 · 30",".75 · 40","1.00 · 50","1.25 · 60","1.50 · 70","1.75 · 80","2.00 · 90","2.25 · 100"].map((x,i)=><div className={i===0?"active":""} key={x}><b>{String(i+1).padStart(2,"0")}</b>{x}</div>)}</div></div></section>
      <section className="section"><div className="mintbox"><div><div className="eyebrow dark">Mint on Arc</div><h2>READY TO CLAIM?</h2><p>Connect your wallet, switch to Arc and mint your raccoon.</p></div><button onClick={() => go("/mint/")}>OPEN MINT ↗</button></div></section>
    </main>
    <Footer go={go}/>
  </>;
}
...
function Footer({ go }) { return <footer className="footer"><span>© 2026 STONKRACCOONS</span><span>ARC MAINNET · NATIVE USDC</span><span className="sociallinks"><a href={X_URL} target="_blank" rel="noreferrer">X ↗</a><a href={OPENSEA_URL} target="_blank" rel="noreferrer">OPENSEA ↗</a></span><button onClick={() => go("/docs/")}>DOCUMENTATION ↗</button></footer>; }
